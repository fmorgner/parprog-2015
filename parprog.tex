...
Vorsicht: Sobald \textbf{wait()} benutzt wird, muss die \textbf{throws InterruptedException} verwendet werden.\\\\
...
class SumTask extends RecursiveTask<Long> {
  ...
  public long compute() {
    if (r - l > THRESHOLD) {
      long m = (l + r) / 2;
      SumTask lTask = new SumTask(array, l, m);
      SumTask rTask = new SumTask(array, m, r);
      lTask.fork(); rTask.fork();
      return lTask.join() + rTask.join();
    } else { // below threshold
      return sequentialTask(array, l, r);
    }
  }
}
\end{lstlisting}
...
\textbf{[volatile in .NET]} verfolgt beim Lesen die sogenannte \textbf{Acquire-Semantik}, was bedeutet, dass alles was \textbf{VOR dem LESEN} einer volatile Variable kommt nacht unten optimiert werden darf. Beim Schreiben wird die \textbf{Release-Semantik} angewandt. Die bedeutet, dass alles was \textbf{NACH dem SCHREIBEN} kommt nach oben optimiert werden darf. Da diese Semantik oft ungenügend ist, gibt es mit \textbf{Thread.MemoryBarrier()} einen "Full-Fence".\\\\
...
void vecAKern(float *A, float *B,
  ...
  // bounds check, array can end before block!
  if(i < N) {
    C[i] = A[i] + B[i];
  }
}
...
void CudaVAdd(float* A, float* B,
  ...
  int bDim = 512; // block size
  int gDim = (N + bDim - 1) / bDim; // grid size
